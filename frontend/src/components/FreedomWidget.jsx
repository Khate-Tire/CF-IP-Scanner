...
export default function FreedomWidget({ onStart, onStop, onSendToAdvanced }) {
    ...
    const handleSubmitConfig = async () => {
        if (!userConfig.trim()) { toast.error(t('freedom.invalidConfig', 'Please paste a valid VLESS config')); return; }
        setSubmitting(true);
        try {
            const res = await provideFreedomConfig(userConfig.trim());
            if (res.error) { toast.error(res.error); }
            else { toast.success(t('freedom.configSubmitted', 'Config submitted! Mining will resume...')); setUserConfig(''); }
        } catch (e) { toast.error(t('freedom.submitFailed', 'Failed to submit config')); }
        ...
        <div className={`mt-6 w-full max-w-4xl mx-auto rounded-3xl overflow-hidden transition-all duration-700 border ${isRunning ? 'border-teal-500/50 shadow-[0_0_30px_rgba(20,184,166,0.3)] bg-black/80' : 'border-white/10 bg-black/40'}`}>
            <div className="flex flex-col md:flex-row">
                ...
                <div className="p-6 md:w-1/3 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-white/10 relative">
                    ...
                    <h3 className="text-xl font-bold text-white mb-1">{t('freedom.playFreedom', 'Play Freedom')}</h3>
                    <p className="text-xs text-gray-400 text-center mb-6">{t('freedom.subtitle', 'Fully autonomous unblockable proxy miner')}</p>
                    ...
                    {!isRunning ? (
                        ...
                        >
                            ...
                            {t('freedom.startAutopilot', 'START AUTOPILOT')}
                        </button>
                    ) : (
                        <button 
                            onClick={onStop}
                ...
                <div className="p-6 md:w-2/3">
                    <div className="mb-4">
                        <h4 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">{t('freedom.currentPhase', 'Current Phase')}</h4>
                        <div className="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-xl">
                            ...
                            <span className={`font-mono text-sm ${isRunning ? 'text-teal-300' : 'text-gray-500'}`}>
                                {status.phase || t('freedom.ready', 'Ready')}
                            </span>
                        </div>
                    </div>
                    ...
                    {needsConfig && (
                        <div className="mb-4 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl">
                            <p className="text-sm text-yellow-400 font-bold mb-2">{t('freedom.noConfig', '⚠️ No working config found. Paste a working VLESS config to continue:')}</p>
                            <div className="flex gap-2">
                                ...
                                >
                                    ...
                                    {submitting ? t('freedom.sending', 'Sending...') : t('freedom.submit', 'Submit')}
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                        {[
                            { key: 'sub', icon: <Radio className="w-5 h-5 text-purple-400 mb-1" />, label: t('freedom.vanillaSub', 'Vanilla Sub OK'), color: 'purple' },
                            { key: 'sub_cf', icon: <Network className="w-5 h-5 text-blue-400 mb-1" />, label: t('freedom.cfSub', 'CF Sub OK'), color: 'blue' },
                            { key: 'spliced', icon: <Cpu className="w-5 h-5 text-pink-400 mb-1" />, label: t('freedom.spliced', 'Spliced Multipliers'), color: 'pink' },
                        ...
                        <div className="flex flex-col items-center p-3 bg-white/5 rounded-xl border border-white/5">
                            ...
                            <span className="text-[10px] text-gray-500 uppercase font-bold text-center">{t('freedom.cleanIps', 'Clean IPs Mined')}</span>
                        </div>
                    </div>
                    ...
                    {expandedSection && (status.found_configs?.[expandedSection]?.length > 0) && (
                        <div className="mt-3 p-3 bg-black/60 border border-white/10 rounded-xl">
                            <div className="flex items-center justify-between mb-2">
                                <span className="text-xs font-bold text-gray-400 uppercase">{t('freedom.foundConfigs', 'Found Configs')} ({status.found_configs[expandedSection].length})</span>
                                <button
                                    onClick={() => {
                                        navigator.clipboard.writeText(status.found_configs[expandedSection].join('\n'));
                                        toast.success(t('freedom.allCopied', 'All configs copied!'));
                                    }}
                                    className="flex items-center gap-1 text-xs text-teal-400 hover:text-teal-300 transition-colors"
                                >
                                    <Copy className="w-3 h-3" /> {t('freedom.copyAll', 'Copy All')}
                                </button>
                            </div>
