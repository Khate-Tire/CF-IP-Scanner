                    ...
                    <div className="flex flex-col items-end gap-2">
                        {authCode ? (
                            <div className="flex flex-col items-center bg-[#2AABEE]/10 border border-[#2AABEE]/30 p-4 rounded-xl min-w-[250px]">
                                <span className="text-xs text-[#2AABEE] font-bold uppercase tracking-wider mb-2">Message Bot This Code</span>
                    ...
                    {(scansCount > 0 || recentScans > 0) && (
                        <div className="max-w-sm w-full bg-gray-900/50 p-4 rounded-xl border border-gray-800 shadow-inner">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-gray-400 text-sm font-semibold">Your Scan Contributions</span>
                                <span className="text-cyan-400 font-bold text-sm">{scansCount.toLocaleString()} total</span>
                            </div>
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-gray-500 text-xs">Recent (last 3 days)</span>
                                <span className="text-orange-400 font-bold text-sm">{recentScans} / 10 IPs</span>
                            </div>
                            ...
                            <p className="text-xs text-gray-500 mt-2">Login with Telegram above to unlock configs</p>
                        </div>
                    )}
                </div>
            ) : (
                            ...
                            {!scanRequirementMet ? (
                                <div className="glass-panel p-8 text-center border-orange-500/30 bg-orange-500/5">
                                    ...
                                    <div className="max-w-md mx-auto mb-6 bg-gray-900/50 p-4 rounded-xl border border-gray-800 shadow-inner">
                                        <div className="flex justify-between items-center mb-2">
                                            <span className="text-gray-400 text-sm font-semibold">Your Recent Contribution</span>
                                            <span className="text-orange-400 font-bold">{recentScans} / 10 IPs</span>
                                        </div>
                                        ...
                                        {recentScans > 0 && <p className="text-xs text-orange-400 animate-pulse mt-2">Almost there! Keep finding those good IPs...</p>}
                                    </div>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <h3 className="text-lg font-bold text-gray-300 flex items-center gap-2">
                                                ...
                                                Community Configs — Mix & Test
                                            </h3>
                                            <p className="text-sm text-gray-500 mt-1">
                                                Picks top 5 fastest CF configs{mixConfigCount > 0 ? ` (from ${mixConfigCount} available)` : ''}, mixes with top 3 found IPs{mixIpCount > 0 ? ` (${mixIpCount} IPs)` : ''}, tests all {mixTotal > 0 ? mixTotal : 15} combos.
                                            </p>
                                        </div>
                                        <button
                                        ...
                                        >
                                            {mixRunning ? (
                                                ...
                                                <><Zap className="w-4 h-4" /> {mixDone ? 'Re-Test All' : 'Mix & Test All'}</>
                                            )}
                                        </button>
                                    </div>
                                        ...
                                        <div className="glass-panel p-4 border-cyan-500/20">
                                            <div className="flex justify-between items-center mb-2">
                                                <span className="text-sm text-gray-400 font-semibold flex items-center gap-2">
                                                    ...
                                                    {mixPhase === 'fetching' && 'Fetching top 5 configs & top 3 IPs...'}
                                                    {mixPhase === 'quick_test' && `Quick ping test — ${mixTested}/${mixTotal} combos${mixConfigCount > 0 ? ` (${mixConfigCount} CF configs available)` : ''}`}
                                                    {mixPhase === 'speed_test' && 'Speed testing top candidates...'}
                                                </span>
                                    ...
                                    {mixTop10.length > 0 && (
                                        <div className="space-y-3">
                                            <h4 className="text-sm font-bold text-yellow-400 flex items-center gap-2">
                                                ...
                                                Top {mixTop10.length} Best Configs (Ranked by Quality)
                                            </h4>
                                            ...
                                            >
                                                <Copy className="w-4 h-4" /> Copy All {mixTop10.length} Configs
                                            </button>
                                        </div>
                                    )}
                                    ...
                                    {!mixRunning && mixTop10.length === 0 && (
                                        <div>
                                            <p className="text-xs text-gray-600 mb-3 mt-4">Or browse pre-injected configs (not speed-tested):</p>
                                            ...
                                            <div key={i} className="bg-black/40 border border-[#2AABEE]/30 rounded-xl p-4 hover:border-[#2AABEE]/80 transition-all group">
                                                <div className="flex justify-between items-start mb-2">
                                                    ...
                                                    <span className="text-xs bg-cyan-500/20 text-cyan-400 px-2 py-1 rounded">Injected IP: {item.clean_ip}</span>
                                                </div>
                                                ...
                                                >
                                                    <Copy className="w-4 h-4" /> Copy Optimized Config
                                                </button>
                                            </div>
                                        )) : (
                                            <div className="col-span-2 text-center text-gray-500 py-8">
                                                Loading configs from Admin Pool...
                                            </div>
                                        )}
                                    </div>
                                    </div>
